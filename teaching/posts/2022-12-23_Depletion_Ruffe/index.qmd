...
4. Convert the effort vector to hours of effort and calculate a new CPE vector. Carefully interpret the meaning of the first value in the CPE vector.
...
1. Convert the estimate of catchability to a quantity that is more easily interpretible. Interpret that quantity.

&nbsp;

8. What is the best estimate for the percentage of the Ruffe population that was removed in all trawling efforts at this site?
...
res2 <- depletion(ct,ft2)
cbind(summary(res2),confint(res2))
plot(res2)

# More interpretible result (b/c q was large)
( q2 <- res2$est["q","Estimate"] )
1-exp(-q2)  # est proportion of popn removed with one unit of effort
```
