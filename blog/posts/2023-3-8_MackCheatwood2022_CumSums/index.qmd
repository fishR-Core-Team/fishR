...
The x-axis labels need to be modified to be monthly, which is accomplished with `date_breaks="month"` within `scale_x_date()`. By default these labels will appear numeric with the month, day, and year. However, just the abbreviated month *name* can be used by including `date_labels="%b%`.^[`%b` is a code the identifies the month name abbreviation. See `?strptime`.]
