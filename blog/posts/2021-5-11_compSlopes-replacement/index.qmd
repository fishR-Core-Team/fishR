...
# What `compSlopes()` Did
The procedure used by `compSlopes()` to compare slopes is described in more detail [here](http://derekogle.com/IFAR/supplements/weightlength/index.html). `compSlopes()` was simple in that it only required the saved `lm()` object as an argument. Its returned results should be assigned to an object for further examination.^[`compSlopes()` had a `print()` function for nicely printing the results. However, here we will look at each component separately to ease comparison with the `emtrends()` results.]
